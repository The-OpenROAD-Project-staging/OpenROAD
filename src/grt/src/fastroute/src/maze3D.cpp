    ...
    do {
      ...
      while (!edges_to_process.empty()) {
        ...
        for (int edgeID : edges_to_process) {
          ...
          while (!pop_heap2_3D_[ind1])  // stop until the grid position been
                                        // popped out from both src_heap_3D and
                                        // dest_heap_3D
          {
            ...
            if (Horizontal) {
              ...
              if (curX > regionX1
                ...
                if (h_edges_3D_[curL][curY][curX - 1].usage + edge_cost
                    ...
                    && net->getMinLayer() <= curL
                    && curL <= net->getMaxLayer()) {
          ...
          // otherwise, no change to subtree1
          {
            ...
            else  // E1 is not on (n1, A1) or (n1, A2), but on (C1, C2)
            {
              ...
              updateRouteType23D(netID,
                                 ...
                                 edge_n1A2,
                                 edge_C1C2);
              // All three edges had their grids overwritten; schedule for
              // retry.
              next_retry.push_back(edge_n1A1);
              next_retry.push_back(edge_n1A2);
            ...
            if (grids[i].layer == grids[i + 1].layer) {
              if (grids[i].x == grids[i + 1].x)  // a vertical edge
              {
                ...
                graph2d_.updateUsageV(
                    grids[i].x, min_y, net, net->getEdgeCost());
              ...
              } else  // a horizontal edge
              {
                ...
                graph2d_.updateUsageH(
                    min_x, grids[i].y, net, net->getEdgeCost());
